...
export function AuctionDetails({
  ...
  const details: [string, string][] = [
    ...
    ['Highest bidder', formatAddress(auction.highest_bidder ?? undefined)],
    ['Starting bid', formatToken(auction.starting_bid)],
    ['Highest bid', formatToken(auction.highest_bid)],
    ['APR', formatAPR(auction.apr_bps)],
    ['Ends', formatDate(auction.auction_end_time)],
    ['Countdown', formatCountdown(auction.auction_end_time)],
  ]

  if (showYield) {
    details.splice(4, 0, ['Estimated yield', formatToken(estimatedYield, 6)])
  }
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.9fr)]">
      <Card className="space-y-6">
        ...
        {auction.dispute_reason ? (
          <Card className="rounded-[24px] bg-rose-500/8">
            <div className="text-sm font-semibold text-rose-200">Dispute reason</div>
